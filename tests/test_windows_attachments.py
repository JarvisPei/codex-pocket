...
from windows_attachments import WindowsAttachmentStore, file_message, safe_path
from windows_bridge import parse_args, make_server
from windows_native import WindowsNativeHandler
...
class FileHandoffTest(unittest.TestCase):
    ...
    def test_windows_directories_inherit_without_special_0700_dacl(self):
        ...
        with patch('windows_attachments.sys.platform', 'win32'), patch.object(Path, 'mkdir', track):
    ...
    def test_workspace_reparse_directory_is_refused(self):
        ...
        a = self.upload()
        retained = self.store.handoff(self.store.resolve([a['id']], 'owner'))
        with patch('windows_attachments.safe_path', side_effect=ValueError('attachment_reparse_point_refused')):
