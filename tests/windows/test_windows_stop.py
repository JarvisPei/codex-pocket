...
from platforms.windows.stop import NativeStopController, validate_stop_payload
...
class StopTest(unittest.TestCase):
    ...
    def test_reader_interrupted_is_not_proof_of_stop(self):
        ...
        with patch('platforms.windows.stop._rollout_activity_snapshot',return_value={'activeTurnId':'turn-a','status':'inProgress'}):
            self.assertEqual(self.stop.stop(THREAD,'turn-a','test')['error'],'native_stop_uncertain')
            self.assertEqual(self.dispatch.call_count,2)
