...
from platforms.windows.bridge import doctor_report, main, parse_args, print_doctor_report
...
class WindowsDoctorTest(unittest.TestCase):
    def setUp(self):
        ...
        self.stack.enter_context(patch('platforms.windows.bridge.state_directory', return_value=self.data))
        self.binary = self.stack.enter_context(patch('platforms.windows.bridge.codex_binary_path', return_value=Path('codex.exe')))
        self.client_type = self.stack.enter_context(patch('platforms.windows.bridge.CodexAppServerClient'))
        self.client = self.client_type.return_value
        self.token = self.stack.enter_context(patch('platforms.windows.bridge.windows_token', return_value='SECRET-DO-NOT-PRINT'))
        self.helper = self.stack.enter_context(patch('platforms.windows.desktop_helper.request_action', return_value={
            'reachable': True, 'sessionId': 1, 'status': 'ready',
            'nativeSendEnabled': True, 'nativeStopEnabled': True,
        }))
        self.http = self.stack.enter_context(patch('platforms.windows.bridge.local_request', return_value={
            'devices': [],
    ...
    def test_json_output_and_exit_status(self):
        ...
        for fails in (False, True):
            ...
            output = io.StringIO()
            with patch('platforms.windows.bridge.sys.platform', 'win32'), contextlib.redirect_stdout(output):
