...
. (Join-Path $root 'platforms\windows\scripts\windows-install-launcher.ps1') -FunctionsOnly
...
try {
    $exe=New-PocketGuiLauncher (Join-Path $root 'platforms\windows\scripts\windows-pocket-launcher.cs') $testRoot
    $bytes=[IO.File]::ReadAllBytes($exe)
    $pe=[BitConverter]::ToInt32($bytes,0x3c)
    Assert-True ([BitConverter]::ToUInt16($bytes,$pe+24+68) -eq 2) 'PE subsystem must be Windows GUI (not Console)'
    $tray=Join-Path $root 'platforms\windows\scripts\windows-pocket-tray.ps1'
