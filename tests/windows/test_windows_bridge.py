...
from platforms.windows.bridge import (
...
class WindowsUnreadApiTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from platforms.windows.bridge import WindowsBridgeServer
    ...
    def test_read_endpoint_uses_windows_acknowledgement_without_json_write(self):
        ...
        with patch('platforms.windows.read_state.mark_read', return_value=True) as mark:
    ...
    def test_unconfirmed_ack_is_error_not_false_success(self):
        with patch('platforms.windows.read_state.mark_read', side_effect=AppServerError('unconfirmed')):
            status, result = self.request('POST', '/api/codex/threads/thread-2/read', {})
        self.assertEqual(status, 502)
        self.assertFalse(result['ok'])

    def test_read_ack_requires_device_auth(self):
        with patch('platforms.windows.read_state.mark_read') as mark:
...
class WindowsLauncherTest(unittest.TestCase):
    ...
    @patch("platforms.windows.bridge.subprocess.run")
    ...
    def test_windows_entry_point_cannot_start_on_mac(self):
        with patch("platforms.windows.bridge.sys.platform", "darwin"), self.assertRaises(SystemExit):
            main(["init"])
