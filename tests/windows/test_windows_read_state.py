...
from platforms.windows.read_state import (STATE_KEY, context_from_auth, identity_key, scope_key,
...
class WindowsReadStateTests(unittest.TestCase):
    ...
    def test_subprocess_payload_has_no_bearer_and_output_is_not_relayed(self):
        with patch('platforms.windows.read_state.subprocess.run', return_value=Mock(returncode=0, stdout='{"sent":true}')) as run:
        ...
        with patch('platforms.windows.read_state.subprocess.run', return_value=Mock(returncode=1, stdout='secret')):
