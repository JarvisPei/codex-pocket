...
from windows_delivery import DeliveryJournal, user_receipts, validate_delivery
...
class DeliveryTest(unittest.TestCase):
    ...
    def test_default_window_accepts_receipt_after_three_seconds(self):
        ...
        def tick(seconds):
            ...
            if len(sleeps) == 16:
                self.data = thread('hello\n')
        with patch('windows_native.time.sleep', side_effect=tick):
            self.assertTrue(self.controller.deliver(self.request_id, THREAD, 'hello')['ok'])
    ...
    def test_draft_guard_checks_verified_destination_not_source_task(self):
        script = (Path(__file__).resolve().parents[1] / 'scripts/windows-desktop-send.ps1').read_text()
    ...
    def test_taskbar_click_is_opt_in_prewrite_and_exact_target_only(self):
        ...
        script = (root / 'scripts/windows-desktop-send.ps1').read_text()
        ...
        module = (root / 'scripts/windows-taskbar-activate.ps1').read_text()
