...
CREDENTIAL_ERROR_CODES = frozenset({
    ...
    'inspect_path', 'read_acl', 'read_owner', 'read_rules', 'inspect_children',
})
...
def windows_token(*, initialize: bool = False) -> str:
    ...
    result = subprocess.run(
        [str(powershell), "-NoLogo", "-NoProfile", "-NonInteractive",
         ...
         str(ROOT / "platforms/windows/scripts/windows-pocket-secret.ps1"),
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(
            ...
            "platforms/windows/scripts/windows-pocket-secret.ps1 -Action Check in PowerShell; "
        ...
        try:
            ...
            if not (data / "token.dpapi").is_file():
                add("credential", "failed", "Pocket credential has not been initialized.",
                    "Run py -3 -m platforms.windows.bridge init in your regular signed-in desktop PowerShell.")
            else:
                token = windows_token()
            ...
            add("credential", "failed", "Existing credential could not be read in this session.",
                "Run platforms/windows/scripts/windows-pocket-secret.ps1 -Action Check in your regular desktop PowerShell. "
            ...
            try:
                ...
                if state.get("reachable") is not True or not isinstance(state.get("sessionId"), int) or state["sessionId"] <= 0:
                    add("helper", "failed", "No fresh helper heartbeat in an interactive desktop session.",
                        "Start platforms/windows/scripts/windows-start-desktop-helper.ps1 with -PythonBinary, -EnableNativeSend and -EnableNativeStop; keep Windows signed in.")
            ...
            try:
                ...
                if capabilities.get("platform") == "windows" and capabilities.get("nativeTextSend") is True:
                    ...
                    add("bridge", "failed", "Local Bridge is not in Windows native-text mode.",
                        "Start python -m platforms.windows.bridge serve with --native-text-send; do not run a second server on the occupied port.")
            except (OSError, RuntimeError, ValueError, subprocess.SubprocessError):
                add("bridge", "failed", "Authenticated loopback Bridge is unavailable.",
                    "Keep python -m platforms.windows.bridge serve --native-text-send running under the same account; check --port.")
...
class WindowsBridgeServer(BridgeServer):
    def project_index(self):
        from platforms.windows.projects import load_windows_project_index
        from platforms.windows.read_state import unread_index
    ...
    def mark_thread_read(self, thread_id, payload):
        from platforms.windows.read_state import mark_read
...
def make_server(binary: Path, token: str, port: int, *, native_text_send: bool = False,
    ...
    from platforms.windows.attachments import WindowsAttachmentStore
    data = state_directory()
    ...
    try:
        ...
        if native_text_send:
            ...
            from platforms.windows.stop import NativeStopController
            ...
            if native_new_tasks:
                from platforms.windows.create import NativeCreateController
