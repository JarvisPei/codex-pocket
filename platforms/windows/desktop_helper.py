...
from platforms.windows.bridge import ROOT, state_directory, windows_token
from platforms.windows.desktop_controls import inspect_composer
from platforms.windows.delivery import validate_delivery
...
def validate_request(request: dict, instance: str, now: float) -> None:
    ...
    if request["action"] in {"native-stop-prepare", "native-stop-commit", "native-resume-prepare", "native-resume-commit"}:
        from platforms.windows.stop import validate_stop_payload
    ...
    try:
        result = subprocess.run(
            [str(powershell), "-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "RemoteSigned",
             "-File", str(ROOT / "platforms/windows/scripts/windows-desktop-diagnostic.ps1"), "-View", "Raw"],
...
def run_smoke_send() -> dict:
    ...
    try:
        completed = subprocess.run(
            [str(powershell), "-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "RemoteSigned",
             "-File", str(ROOT / "platforms/windows/scripts/windows-desktop-smoke-send.ps1")],
            capture_output=True, encoding="utf-8", timeout=25,
            creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0),
        )
...
def run_native_send(delivery: dict, *, allow_display_off: bool = False, allow_taskbar_activation: bool = False) -> dict:
    ...
    try:
        completed = subprocess.run(
            [str(powershell), "-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "RemoteSigned",
             "-File", str(ROOT / "platforms/windows/scripts/windows-desktop-send.ps1"),
...
def run_native_stop(delivery: dict, phase: str, *, resume: bool = False) -> dict:
    ...
    try:
        completed = subprocess.run([str(powershell), "-NoLogo", "-NoProfile", "-NonInteractive",
            "-ExecutionPolicy", "RemoteSigned", "-File", str(ROOT / "platforms/windows/scripts/windows-desktop-send.ps1"),
